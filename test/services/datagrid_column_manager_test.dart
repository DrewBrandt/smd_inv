...
void main() {
  group('DataGridColumnManager', () {
    ...
    test('calculateWidths starts with minimum widths', () {
      ...
      final widths = manager.calculateWidths(
    ...
    test(
      'calculateWidths distributes extra space proportionally by weight',
      () {
        final columns = [
    ...
    test(
      'calculateWidths enforces minimum widths on saved preferences',
      () async {
        final columns = [
    ...
    test('calculateWidths handles infinite width constraint', () {
      final columns = [const GridColumnConfig(field: 'notes', label: 'Notes')];
      final manager = DataGridColumnManager(
        persistKey: 'test',
        columns: columns,
      );
    ...
    test(
      'calculateWidths gives extra space to last column when no growable columns',
      () async {
        final columns = [
          const GridColumnConfig(
            field: 'datasheet',
    ...
    test('loadSavedWidths and saveWidths persist preferences', () async {
      ...
      final manager2 = DataGridColumnManager(
        persistKey: 'test_persist',
        columns: columns,
      );
      await manager2.loadSavedWidths();

      final widths = manager2.calculateWidths(
        const BoxConstraints(maxWidth: 1000),
      );
    ...
    test('isResizing prevents width redistribution during resize', () async {
      ...
      final widthsDuringResize = manager.calculateWidths(
        const BoxConstraints(maxWidth: 1000),
      );
      ...
      final widthsAfterResize = manager.calculateWidths(
        const BoxConstraints(maxWidth: 1000),
      );
      expect(widthsAfterResize['part_#'], 300);
      expect(
        widthsAfterResize['notes'],
        closeTo(700, 0.1),
